...
//  Swift-FireWork
//
//  Created by xuguangyin on 2021/12/14.
...
public extension UIView {
    ...
    func showToast(_ toast: UIView, duration: TimeInterval = ToastManager.shared.duration, point: CGPoint, completion: ((_ didTap: Bool) -> Void)? = nil) {
        ...
        func hideToastActivity(_ activity: UIView) {
            guard activeToasts.contains(activity) else { return }
            hideToast(activity, fromTap: false)
        }
...
//            makeToastActivity(toast, point: point, duration: ToastManager.shared.duration)
            print("222")
        }
        ...
        private func makeToastActivity(_ toast: UIView, point: CGPoint,  duration: TimeInterval) {
            toast.alpha = 0.0
            toast.center = point
        ...
        private func activityTimerDidFinish(_ timer: Timer) {
            guard let activity = timer.userInfo as? UIView else { return }
            hideToastActivity()
