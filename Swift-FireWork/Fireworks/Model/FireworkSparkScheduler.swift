...
public class FireworkSparkScheduler {
    ...
    public func schedule(sparks: [FireworkSpark], in presenterView: UIView, with animator: SparkViewAnimator, animationDuration: TimeInterval) {
        let data = Data(presenterView: presenterView, sparks: sparks, animator: animator, animationDuration: animationDuration)
    ...
    public func scheduleTimer(){
        ...
        self.timer = Timer.scheduledTimer(timeInterval: self.delay, target: self, selector: #selector(timerDidFire), userInfo: nil, repeats: false)
    }
